...
async function provisionEmployer(
  ...
  const { data: existing } = await supabase
    .from("groups")
    .select("id, invite_code, stripe_subscription_id")
    .eq("stripe_customer_id", customerId)
    .maybeSingle();

  let groupId: string;
  let inviteCode: string;
  // Stripe redelivers events (retries after outages, manual resends). DB
  // writes below are idempotent, but the welcome email must only go out
  // ONCE — on the delivery that first records this subscription.
  let isReplay = false;

  if (existing) {
    isReplay = existing.stripe_subscription_id === subscriptionId;
    ...
    console.log(`[EmployerWebhook] Updated existing group ${groupId}${isReplay ? " (replay)" : ""}`);
    ...
    const { data: adoptee } = await supabase
      .from("groups")
      .select("id, name, invite_code, stripe_customer_id, stripe_subscription_id")
    ...
    isReplay = adoptee.stripe_subscription_id === subscriptionId;
  ...
  if (isReplay) {
    console.log(
      `[EmployerWebhook] Replayed delivery for subscription ${subscriptionId} — welcome email already sent, skipping`,
    );
    return;
  }
