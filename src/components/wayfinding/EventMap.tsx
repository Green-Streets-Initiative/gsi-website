...
import { fetchBluebikes, fetchMBTAStops, fetchTrainStops, fetchBikeParking } from '@/lib/nearby/live-data'
...
export default function EventMap({
  ...
  const markerIcons = {
    ...
    storefront: '<svg width="16" height="16" viewBox="0 0 256 256" fill="white"><path d="M232,96a7.89,7.89,0,0,0-.3-2.2L217.35,43.6A16.07,16.07,0,0,0,202,32H54A16.07,16.07,0,0,0,38.65,43.6L24.31,93.8A7.89,7.89,0,0,0,24,96h0v16a40,40,0,0,0,16,32v72a8,8,0,0,0,8,8H208a8,8,0,0,0,8-8V144a40,40,0,0,0,16-32V96ZM54,48H202l11.42,40H42.61Zm50,56h48v8a24,24,0,0,1-48,0Zm-16,0v8a24,24,0,0,1-35.12,21.26,7.88,7.88,0,0,0-1.82-1.06A24,24,0,0,1,40,112v-8ZM200,208H56V151.2a40.57,40.57,0,0,0,8,.8,40,40,0,0,0,32-16,40,40,0,0,0,64,0,40,40,0,0,0,32,16,40.57,40.57,0,0,0,8-.8Zm4.93-75.8a8.08,8.08,0,0,0-1.8,1.05A24,24,0,0,1,168,112v-8h48v8A24,24,0,0,1,204.93,132.2Z"/></svg>',
    wrench: '<svg width="16" height="16" viewBox="0 0 256 256" fill="white"><path d="M226.76,69a8,8,0,0,0-12.84-2.88l-40.3,37.19-17.23-3.7-3.7-17.23,37.19-40.3A8,8,0,0,0,187,29.24,72,72,0,0,0,88,96,72.34,72.34,0,0,0,94,124.94L33.79,177c-.15.12-.29.26-.43.39a32,32,0,0,0,45.26,45.26c.13-.13.27-.28.39-.42L131.06,162A72,72,0,0,0,232,96,71.56,71.56,0,0,0,226.76,69ZM160,152a56.14,56.14,0,0,1-27.07-7,8,8,0,0,0-9.92,1.77L67.11,211.51a16,16,0,0,1-22.62-22.62L109.18,133a8,8,0,0,0,1.77-9.93,56,56,0,0,1,58.36-82.31l-31.2,33.81a8,8,0,0,0-1.94,7.1L141.83,108a8,8,0,0,0,6.14,6.14l26.35,5.66a8,8,0,0,0,7.1-1.94l33.81-31.2A56.06,56.06,0,0,1,160,152Z"/></svg>',
  }
  ...
  async function fetchLiveData(_map: maplibregl.Map) {
    ...
    // Shared fetchers live in @/lib/nearby/live-data (same behavior + cache keys)
    const bbPromise = fetchBluebikes(event.center_lat, event.center_lng).then(r => { bb = r; onLiveDataLoad(bb, mbta, bp, train) })
    const mbtaPromise = fetchMBTAStops(event.center_lat, event.center_lng).then(r => { mbta = r; onLiveDataLoad(bb, mbta, bp, train) })
    const bpPromise = fetchBikeParking(event.center_lat, event.center_lng).then(r => { bp = r; onLiveDataLoad(bb, mbta, bp, train) })
    const trainPromise = fetchTrainStops(event.center_lat, event.center_lng).then(r => { train = r; onLiveDataLoad(bb, mbta, bp, train) })
