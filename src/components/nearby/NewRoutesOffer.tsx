...
export default function NewRoutesOffer({
  ...
  const eyebrow = (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[0.62rem] font-bold uppercase tracking-[0.16em] text-(--nb-accent)">
  ...
  const cta = (
    ...
    <a
      href={href}
      onClick={onCta}
      className="inline-flex shrink-0 items-center rounded-lg bg-(--nb-accent-fill) px-4 py-2 text-[0.8rem] font-bold text-(--nb-on-accent-fill) transition-opacity hover:opacity-85"
    >
  ...
  if (variant === 'compact') {
    return (
      <div className="rounded-xl border border-(--nb-accent-line) bg-[image:var(--nb-promo-bg)] px-4 py-3 text-left">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            {eyebrow}
            <div className="mt-1 text-[0.95rem] font-bold leading-tight text-(--nb-ink)">
              {tr('newroutes.offer_title')}
            </div>
            <p className="mt-0.5 text-[0.78rem] leading-snug text-(--nb-ink-80)">
  ...
  return (
    <div className="rounded-2xl border border-(--nb-accent-line) bg-[image:var(--nb-promo-bg)] px-5 py-4 text-left">
      {eyebrow}
      <div className="mt-1.5 font-display text-[1.05rem] font-extrabold leading-tight text-(--nb-ink)">
        {tr('newroutes.offer_title')}
      </div>
      <p className="mt-1 text-[0.82rem] leading-snug text-(--nb-ink-80)">{tr('newroutes.offer_body')}</p>
      <div className="mt-2.5 flex items-center gap-2">
        <span className="shrink-0 rounded-md border border-(--nb-accent-line) bg-(--nb-accent-tint) px-2 py-1 font-mono text-[0.8rem] font-bold tracking-wider text-(--nb-accent)">
          {NEWROUTES_CODE}
        </span>
        <span className="text-[0.72rem] leading-snug text-(--nb-ink-70)">{tr('newroutes.offer_code_note')}</span>
      </div>
