...
export function renderCeSpotlightHero(imageUrl: string | null, eventType = ''): string {
  if (imageUrl && imageUrl.trim()) {
    return `<img src="${escapeHtml(imageUrl)}" alt="" style="width:1080px;height:1180px;object-fit:cover;display:block" />`;
  }
  // No flyer: branded hero — tinted radial glow in the event type's color
  // with a large watermark icon, so the card still posts cleanly.
  const meta = ceTypeMeta(eventType);
  return `
    <div style="width:1080px;height:1180px;background:radial-gradient(circle at 50% 42%, ${ceTint(meta.color, 0.22)} 0%, rgba(31,32,52,0) 62%), #1F2034;display:flex;align-items:center;justify-content:center">
      <div style="width:420px;height:420px;border-radius:96px;display:flex;align-items:center;justify-content:center;background:${ceTint(meta.color, 0.14)};border:3px solid ${ceTint(meta.color, 0.35)}">
        ${ceIcon(meta.icon, 220, 1.5, meta.color)}
      </div>
    </div>`;
}
