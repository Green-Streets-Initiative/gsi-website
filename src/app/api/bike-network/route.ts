 ...
 *
 *  - MAPC TrailMap (ArcGIS, keyless): protected lanes / shared-use paths /
 *    painted lanes as separate "existing" layers with local names. Adapted
 *    from the Shift repo's _shared/mapc.ts, but returning full polylines
 *    instead of sampled points.
 *  - MassDOT BikeInventory (ArcGIS, keyless): the official statewide
 *    inventory — covers areas MAPC misses. Fac_Type 2/5 = separated,
 *    1/4 = painted; other codes (sharrows, signage) are excluded from
 *    drawing to keep the map honest.
 *  - OpenStreetMap Overpass: freshest data — new lanes usually land in OSM
 *    before official inventories. Primary + mirror, tolerant timeout.
 *
 * Sources are queried in parallel; any failure degrades to an empty list.
 * Overlapping duplicates between sources are drawn on top of each other in
...
type Quality = 'separated' | 'painted'
...
export async function GET(req: NextRequest) {
  ...
  // Round to ~110 m so cache keys coincide across visitors in the same area
